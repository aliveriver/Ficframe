...
- 稳定原文引用：每条分镜保存原文区间、引用快照和上下文锚点；正文坐标变化时会尝试可靠重定位，无法定位时不会错误高亮。
- 分镜 Agent：反馈和对话随 run 保存，Agent 可根据反馈决定是否重生成对应分镜；Prompt 重建意见使用独立历史。
- 可观察任务：完整工作流、分镜重生成、Prompt 重建和图片生成都会显示阶段、进度、最近消息与失败原因。
- 分镜历史：手动修改、删除、Agent 重生成和版本恢复前会归档尚未保存的文本版本；相同内容不会重复入库，且不影响图片历史。
...
9. 设置分镜数量和图片尺寸。
10. 点击 `生成分镜`，在顶部任务面板查看阶段、进度和失败原因。
11. 通过 `预览小说` 查看引用高亮，或选中原文/填写描述新增分镜。
...
`pipeline.json` 是 run 的规范业务状态；`tasks.json` 保存该 run 最近的后台任务快照。`storyboard.md`、`prompts.md` 和导出文件都是可重新生成的投影。
...
需要结构化结果时，Responses 请求会按照 DeepSeek 官方格式发送 `text: {"format": {"type": "json_object"}}`，并为推理与最终 JSON 共用的 `max_output_tokens` 预留足够空间。若响应状态为 `incomplete`，系统会报告截断或内容过滤原因，不会把不完整 JSON 写入 run。
...
- 每条分镜保存对应小说原文及字符区间；点击 `查看原文` 会在小说预览窗口中高亮生成依据。
- 分镜详情可直接修改并保存，也可删除；点击 `添加分镜` 后，可以选中小说段落或用文字描述让 LLM 新增分镜。
- `分镜 Agent` 会把用户反馈和回复保存在当前 run 的 `pipeline.json` 中，并自主判断是否需要立即重生成、应修改哪些分镜；纯表扬或无需修改时不会误触发。
- 手动保存、Agent 自动重生成、批量重生成和版本恢复会保留尚未归档的上一版分镜文本，可在详情区查看并恢复；相同文本快照只保留一份，分镜版本与图片版本相互独立。
- 重新生成分镜只更新分镜文本、构图和 Prompt，不会清空该分镜已有的当前图片或图片版本。

实现边界和扩展约定见 [分镜工作流架构](docs/storyboard-architecture.md)。
